...
def remove_all_with_difference_less_than_23(cycles,limits=50):
    count=0
    ...
    for k_mer in cycles.keys():
        ...
        for cycle in cycles[k_mer]:
            if len(cycle) - common_elements <= 25 or len(cycle) - common_elements > limits:
                count+=1
            else:
                value.append(cycle)
        if len(value) > 0:
...
#/vol/d/development/git/mCAAT/proof_of_concept/data/metagenome_03/graph_27
