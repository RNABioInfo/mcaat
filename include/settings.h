...
struct Settings {
    ...
    std::string output_file = "out/CRISPR_Arrays.txt";
    ...
    std::map<std::string, std::pair<bool, std::string>> SettingsMap(double total_in_b, int threads){
        ...
        settings_message_map["RAM"] =this->ram >1 && this->ram <= total_in_b ? make_pair(true, ram_to_check+" B") : make_pair(false, "value "+ram_to_check+" is more " + "than total "+ total_ram_string+" amount of RAM(in Bytes)");
    ...
    std::string PrintSettings()
    {
        ...
        double total_in_b = memory.total_Bytes();
