...
double Profile::GetMaxEmissionScore() const {
    // Returns the maximum LOG-ODDS emission score (in bits) any amino acid can achieve
    // Used for early termination bounds in BeamSearch
    // Must match the scoring used in Viterbi (GetMatchLogOdds converted to bits)
    
    double max_bits = -1e9;
    const char amino_acids[] = "ACDEFGHIKLMNPQRSTVWY";
    
    for (int pos = 1; pos <= static_cast<int>(states_.size()); ++pos) {
        for (char aa : amino_acids) {
            double log_odds = GetMatchLogOdds(pos, aa);
            double bits = log_odds / std::log(2.0);  // Convert to bits
            if (bits > max_bits) {
                max_bits = bits;
            }
        }
    ...
    // Return a reasonable default if no valid scores found
    return (max_bits > -1e8) ? max_bits : 3.0;
}
